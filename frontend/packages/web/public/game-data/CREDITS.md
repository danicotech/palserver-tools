...
- **Pal icons** (`pals/`): bundled with this project.
...
- **Passive-skill catalog** (`passives.json`): internal ids, names and ranks
  from [paldeck.cc](https://paldeck.cc).
...
- **Guild Lab Research catalog** (`research.json`): labels the save file's
  ...
  full coverage). **Caveat**: that repo has no stated license —
  re-review before treating this as
...
Tree map tiles (`node scripts/fetch-worldtree-map.mjs`, same arrangement as the
game-data name sync).
