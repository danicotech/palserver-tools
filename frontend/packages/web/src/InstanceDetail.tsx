...
export function InstanceDetailPage({
  ...
  // 手機/平板:分頁改用側邊漢堡抽屜(桌機 lg+ 維持可拖曳分頁列)
    ...
    <div className="flex flex-col gap-4">
      ...
      {showConsole && (
        <Overlay onClose={() => setShowConsole(false)}>
          <div
            className={`${card} flex h-[82dvh] w-240 max-w-full flex-col gap-3 overflow-hidden`}
      ...
      {showLogs && (
        <Overlay onClose={() => setShowLogs(false)}>
          <div
            className={`${card} flex max-h-[90dvh] w-240 max-w-full flex-col gap-3 overflow-y-auto`}
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex shrink-0 items-center justify-between">
              <h2 className="inline-flex items-center gap-2 text-lg font-extrabold">
      ...
      {(() => {
        ...
        const activeLabel = translate((visibleTabs.find((tb) => tb.id === tab) ?? TABS.find((tb) => tb.id === tab))?.label ?? "");
        return (
          <>
            {/* 手機/平板:漢堡列(顯示目前分頁);桌機(lg+):可拖曳分頁列。 */}
            <button
              type="button"
              onClick={() => setNavOpen(true)}
              className="flex min-h-12 w-full items-center gap-3 rounded-xl border-2 border-line bg-card-soft px-4 py-2.5 text-left font-extrabold text-ink transition hover:border-pal lg:hidden"
              aria-label={translate("開啟分頁選單")}
              aria-expanded={navOpen}
            >
              <span className="text-xl leading-none">☰</span>
              <span className="min-w-0 flex-1 truncate">{activeLabel}</span>
              <span className="shrink-0 text-xs font-medium text-ink-muted">{translate("切換分頁")}</span>
            </button>

            {/* 桌機(lg+):可拖曳分頁列 + 管理面板 */}
            <div className="hidden flex-wrap gap-x-2 gap-y-1 border-b-2 border-line lg:flex">
            ...
            {/* 手機/平板側邊漢堡抽屜:切換分頁 + 管理(顯示/隱藏)分頁 */}
            {navOpen && (
              <div className="fixed inset-0 z-50 lg:hidden" role="dialog" aria-modal="true" aria-label={translate("分頁選單")}>
                <div className="absolute inset-0 bg-[rgb(35_32_48/0.55)] backdrop-blur-[2px]" onClick={() => setNavOpen(false)} />
                <div className="drawer-in absolute inset-y-0 left-0 flex w-72 max-w-[82vw] flex-col bg-card shadow-xl">
                  <div className="flex items-center justify-between border-b-2 border-line px-4 py-3">
                    <span className="text-base font-extrabold text-ink">{translate("分頁選單")}</span>
                    <button
                      type="button"
                  ...
                  <nav className="flex-1 space-y-1 overflow-y-auto p-2">
                    {visibleTabs.map((tb) => (
                      <button
                        ...
                        className={`flex min-h-12 w-full items-center rounded-xl px-3 py-2.5 text-left text-base font-bold transition ${
                          tb.id === tab ? "bg-pal text-white shadow-(--shadow-cute)" : "text-ink hover:bg-card-soft"
                        }`}
                      >
                        {translate(tb.label)}
                      </button>
                    ...
                    {manageable.length > 0 && (
                      <div className="mt-2 border-t-2 border-line pt-2">
                        ...
                        {manageable.map((tb) => {
                          ...
                          return (
                            <button
                              key={tb.id}
                              type="button"
                              className={`flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm font-bold transition hover:bg-card-soft ${shown ? "" : "text-ink-muted"}`}
                              onClick={() => {
                                if (shown) {
                                  setHiddenTabs([...hiddenTabs, tb.id]);
                                } else {
                            ...
                            >
                              {shown ? <FiCheck className="size-3.5 shrink-0 text-pal" /> : <FiPlus className="size-3.5 shrink-0" />}
                              {translate(tb.label)}
                            </button>
                          );
                        })}
                        <button
                          type="button"
                          className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-xs font-bold text-ink-muted transition hover:bg-card-soft hover:text-ink"
                          onClick={() => {
                            setHiddenTabs(defaultHiddenTabs(enhancedMode));
                            setTabOrder(TABS.map((tb) => tb.id));
                            setNavOpen(false);
