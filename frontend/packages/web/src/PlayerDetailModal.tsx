...
export function PlayerDetailModal({
  ...
  return (
    <Overlay onClose={onClose}>
      <div
        className={`${card} flex max-h-[85dvh] w-[720px] max-w-full flex-col gap-4 overflow-y-auto`}
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex flex-wrap items-center justify-between gap-2">
          <h2 className="min-w-0 truncate text-lg font-extrabold">{displayLabel}</h2>
  ...
  return (
    <div>
      <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
        ...
        <input
          className={`${inputCls} w-full py-1 text-base sm:w-44 sm:text-xs`}
      ...
      <div className="flex items-center gap-2.5">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-0.5 flex flex-wrap items-center gap-x-1.5 text-xs text-ink-muted">
            ...
            {p.shiny && (
              <span className="rounded-full bg-amber-400/15 px-1.5 font-bold text-amber-500">✦ {t("稀有")}</span>
            )}
            {p.isBoss && (
