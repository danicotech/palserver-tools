    ...
    <div
      className={
        fullscreen
          ...
          : // 手機:近全螢幕高、鋪滿寬度(工具列會換行,方形會把地圖擠到很小);sm 以上回到方形。
            "flex h-[88vh] max-h-[88dvh] w-[92vw] max-w-full flex-col gap-2 rounded-(--radius-cute) border-2 border-line bg-card p-3 shadow-(--shadow-cute) sm:h-[min(88vh,92vw)] sm:w-[min(88vh,92vw)]"
      }
...
function GuildDetailModal({
  ...
  return (
    <Overlay onClose={onClose}>
      <div
        className={`${card} flex max-h-[85dvh] w-[560px] max-w-full flex-col gap-4 overflow-y-auto`}
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between gap-2">
