      ...
      <div className="grid min-h-0 flex-1 gap-3 sm:grid-cols-[220px_minmax(0,1fr)]">
        ...
        <div className="flex min-h-0 flex-col gap-2 rounded-cute border-2 border-line p-2 max-sm:max-h-52">
          ...
          <div className="min-h-0 flex-1 overflow-y-auto">
            {/* PalDefender 進階指令:混在清單裡跟其他分類一樣捲動,星星標示。 */}
            {SHOW_SPONSOR_FEATURES && catalog.paldefender && (
              <div>
                <p className="mt-2 mb-1 px-1 text-xs font-extrabold text-ink-muted">{t("進階指令")}</p>
        ...
        <div className="flex min-h-0 flex-col gap-3">
          ...
          >
            ...
            <input
              className="min-w-0 flex-1 bg-transparent font-mono text-base outline-none sm:text-sm"
              value={raw}
