...
/** 公開版:所有功能免費開放,已無贊助鎖定提示(保留匯出以相容呼叫端,渲染為空)。 */
export function SponsorLockNotice(_props: { children?: ReactNode }) {
  return null;
}
...
// 手機優先:py-2.5 讓觸控目標更接近 44px,桌機視覺變動極小。
export const btn =
  "rounded-full bg-pal px-5 py-2.5 text-sm font-extrabold text-white transition " +
  "hover:-translate-y-px hover:bg-pal-strong active:translate-y-0 active:scale-95 " +
  "disabled:pointer-events-none disabled:opacity-50";
export const btnGhost =
  "rounded-full border-2 border-line bg-card-soft px-5 py-2.5 text-sm font-extrabold text-ink " +
...
export const btnSponsor =
  "rounded-full bg-sponsor px-5 py-2.5 text-sm font-extrabold text-white transition " +
  "hover:-translate-y-px hover:brightness-95 active:translate-y-0 active:scale-95 " +
  "disabled:pointer-events-none disabled:opacity-50";
export const card = "rounded-(--radius-cute) border-2 border-line bg-card p-4 shadow-(--shadow-cute) sm:p-5";
// text-base(16px)於手機防止 iOS 對焦自動放大;桌機回到 14px 保持密度。
export const inputCls =
  "rounded-xl border-2 border-line bg-card-soft px-3 py-2 text-base text-ink outline-none " +
  "transition focus:border-pal sm:text-sm";
...
export function Overlay({ children, onClose }: { children: React.ReactNode; onClose: () => void }) {
  // 開啟時鎖住背景捲動(手機上避免彈窗背後的頁面跟著滑動);ESC 可關閉。
  useEffect(() => {
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-[rgb(35_32_48/0.55)] p-3 backdrop-blur-[3px] sm:p-6"
      onClick={onClose}
    >
      {children}
    </div>
...
/** 公開版:詳細資訊全開放,已無鎖提示(保留匯出以相容呼叫端,渲染為空)。 */
