...
function AddressChip({ address, secret }: { address: string; secret?: boolean }) {
  ...
  return (
    <button
      onClick={copy}
      className="inline-flex max-w-full items-center gap-2 rounded-lg border-2 border-line bg-card-soft px-3 py-1.5 font-mono text-sm font-bold transition hover:border-pal"
      title={t("點擊複製")}
    >
      <span className={`min-w-0 break-all ${hidden ? "select-none blur-[6px]" : ""}`}>{address}</span>
      {secret && (
        <span
          role="button"
          tabIndex={0}
