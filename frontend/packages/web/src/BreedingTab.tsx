...
import { FiCrosshair, FiGitBranch, FiMapPin, FiMaximize2, FiRefreshCw, FiSearch, FiTable, FiZoomIn, FiZoomOut } from "react-icons/fi";
...
import { BreedingTablePanel } from "./BreedingTablePanel";
...
export function BreedingTab({ client, instanceId, onShowOnMap }: { client: AgentClient; instanceId: string; onShowOnMap?: (x: number, y: number) => void }) {
  ...
  /** 「配種表」是純靜態查表,不需存檔快照也不需贊助者識別碼;「配種路徑」才要。 */
  const [view, setView] = useState<"table" | "solver">("table");
  ...
  const load = useCallback(async () => {
    setLoading(true);
    // 配方表與存檔快照分開載:breeding.json 是靜態檔,存檔還沒掃描(或 agent 斷線)
    // 也不該讓「配種表」跟著不能用 —— 舊版把兩者放同一個 Promise.all,快照一失敗
    // 整包就 catch 掉,breedingData 永遠是 null。
    try {
      setBreedingData(await loadBreedingData());
    } catch (err) {
    ...
    try {
      const snapshot = await client.breedingSnapshot(instanceId);
  ...
  if (!breedingData) return <p className={errorCls}>{error ?? t("無法載入配種資料。")}</p>;
    ...
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap gap-1.5">
        <button
          ...
          className={`inline-flex items-center gap-1.5 rounded-full border-2 px-4 py-2 text-sm font-extrabold transition ${
            view === "table" ? "border-pal bg-pal/10 text-pal" : "border-line bg-card-soft text-ink-muted hover:border-pal"
          }`}
        >
          <FiTable className="size-4" /> {t("配種表查詢")}
        </button>
        <button
          type="button"
          onClick={() => setView("solver")}
          className={`inline-flex items-center gap-1.5 rounded-full border-2 px-4 py-2 text-sm font-extrabold transition ${
            view === "solver" ? "border-pal bg-pal/10 text-pal" : "border-line bg-card-soft text-ink-muted hover:border-pal"
          }`}
        >
          <FiGitBranch className="size-4" /> {t("配種路徑計算")}
          {entitled === false && <span title={t("贊助者先行版功能")}>🔒</span>}
        </button>
      </div>

      {view === "table" && <BreedingTablePanel data={breedingData} gameData={gameData} />}

      {view === "solver" && entitled === false && (
