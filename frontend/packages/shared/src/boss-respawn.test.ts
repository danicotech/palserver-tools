...
test("bossRespawnInfo:野外頭目一律「下個遊戲日」,即使有實測 respawnInterval 也忽略、不算倒數", () => {
  const died = 5000;
  const r = bossRespawnInfo(entry({ alive: false, diedAt: died, respawnInterval: 1800 }), died + 100);
  assert.equal(r.status, "dead");
  assert.equal(r.measured, false);
  assert.equal(r.respawnAt, null);
  assert.equal(r.secondsLeft, null);
...
test("bossRespawnInfo:陳舊的巨大 respawnInterval 不再算出離譜倒數(回歸:22:00:00 bug)", () => {
  const died = 1000;
  // 79189 秒 ≈ 22 小時,曾因陳舊實測值害倒數顯示 22:00:00;現在一律忽略 respawnInterval。
  const r = bossRespawnInfo(entry({ alive: false, diedAt: died, respawnInterval: 79189 }), died + 30);
  assert.equal(r.status, "dead");
  assert.equal(r.secondsLeft, null);
  assert.equal(r.measured, false);
});
...
test("bossRespawnInfo:野外頭目擊殺超過寬容期沒被觀測到復活 → 退回未知(即使帶 respawnInterval,回歸測試)", () => {
  const died = 1000;
  // 寬容期(45 分)內:仍算已擊殺、顯示下個遊戲日(secondsLeft 恆 null,不算倒數)。
  const inGrace = bossRespawnInfo(entry({ alive: false, diedAt: died, respawnInterval: 1200 }), died + 44 * 60);
  assert.equal(inGrace.status, "dead");
  assert.equal(inGrace.secondsLeft, null);
  // 超過寬容期(以 diedAt 起算):退回未知,不再永遠卡在已擊殺。
  const overGrace = bossRespawnInfo(entry({ alive: false, diedAt: died, respawnInterval: 1200 }), died + 45 * 60 + 1);
  assert.equal(overGrace.status, "unknown");
  assert.equal(overGrace.secondsLeft, null);
});
...
test("buildPublicMapBossPoints:野外頭目 dead 帶 respawnInterval 也忽略 → st=dead、不帶 ra/ms(不做倒數)", () => {
  const state = mkState({
