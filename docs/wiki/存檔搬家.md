...
網站上的**玩家、帕魯、公會與據點全部來自同一個檔** —— 世界資料夾裡的 `Level.sav`。
系統挑檔的順序是:先看 `Pal/Saved/Config/LinuxServer/GameUserSettings.ini` 的
`DedicatedServerName=` 指到哪個資料夾,找不到才退而挑 `SaveGames/0/` 底下**最新修改**的那份。
解析結果依 `Level.sav` 的修改時間快取,存檔沒變就不重解。
...
| 本專案的 [[SteamCMD 版]] | `windows/native/server/Pal/Saved/SaveGames/0/<GUID>` |

## 分平台的圖解

| 平台 | 圖解 |
|---|---|
| Windows | ![Windows 搬家](svg/migrate-windows.svg) |
| Linux / macOS | ![Linux 搬家](svg/migrate-linux.svg) |

(同樣的內容也寫在 `windows/README.md` 與 `linux/README.md`,含可直接複製的指令)
...
| 方向 | 從 | 到 |
|---|---|---|
| SteamCMD 版 → Docker 版 | `windows/native/server/Pal/Saved/SaveGames/0/<GUID>` | `backend/palworld-data/Pal/Saved/SaveGames/0/` |
| Docker 版 → SteamCMD 版 | `backend/palworld-data/.../0/<GUID>` | `windows/native/server/Pal/Saved/SaveGames/0/` |
...
> 遊戲性設定不在存檔裡:Docker 版改 `.env`(見 [[伺服器參數 .env]]);SteamCMD 版改 `PalWorldSettings.ini`。
